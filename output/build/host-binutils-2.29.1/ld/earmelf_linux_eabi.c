...
static void gldarmelf_linux_eabi_after_parse (void);
...
static void
gldarmelf_linux_eabi_after_parse (void)
{
...
static bfd_boolean
gldarmelf_linux_eabi_check_ld_so_conf (const struct bfd_link_needed_list *l,
				       int force)
{
  ...
  if (! initialized)
    {
      ...
      tmppath = concat (ld_sysroot, "/home/epl/Jan_7/Linux-Test-Framework/output/host/etc/ld.so.conf",
			(const char *) NULL);
      if (!gldarmelf_linux_eabi_parse_ld_so_conf (&info, tmppath))
	{
      ...
      if (info.path)
	{
	  ld_so_conf = gldarmelf_linux_eabi_add_sysroot (info.path);
	  free (info.path);
	}
      initialized = TRUE;
    }
...
gldarmelf_linux_eabi_after_open (void)
{
  ...
  for (l = needed; l != NULL; l = l->next)
      ...
      for (force = 0; force < 2; force++)
	{
	  ...
	  if (path)
	    {
	      path = gldarmelf_linux_eabi_add_sysroot (path);
	      found = gldarmelf_linux_eabi_search_needed (path, &n, force);
	      free ((char *) path);
	      if (found)
		break;
	    }
	  ...
	  for (; !found && rp != NULL; rp = rp->next)
	    {
	      path = gldarmelf_linux_eabi_add_sysroot (rp->name);
	      found = (rp->by == l->by
		       && gldarmelf_linux_eabi_search_needed (path, &n,
							      force));
	      free ((char *) path);
	    }
	  if (found)
	  ...
	  if (gldarmelf_linux_eabi_check_ld_so_conf (l, force))
	    break;
...
static void
gldarmelf_linux_eabi_append_to_separated_string (char **to, char *op_arg)
{
  if (*to == NULL)
    *to = xstrdup (op_arg);
  else
	...
	if (audit_libs && *audit_libs != '\0')
	  {
		...
		if (cp != NULL && *cp != '\0')
		  gldarmelf_linux_eabi_append_to_separated_string (&depaudit, cp);
...
OUTPUT_ARCH(arm)\n\
ENTRY(_start)\n\
SEARCH_DIR(\"=/home/epl/Jan_7/Linux-Test-Framework/output/host/arm-buildroot-linux-uclibcgnueabihf/lib\"); SEARCH_DIR(\"=/usr/local/lib\"); SEARCH_DIR(\"=/lib\"); SEARCH_DIR(\"=/usr/lib\");\n\
SECTIONS\n\
{\n\
  /* Read-only sections, merged into text segment: */\n\
...
gldarmelf_linux_eabi_handle_option (int optc)
{
  switch (optc)
	...
	einfo (_("%P%F: invalid --compress-debug-sections option: `%s'\n"),
      ...
      break;
    case OPTION_AUDIT:
	gldarmelf_linux_eabi_append_to_separated_string (&audit, optarg);
	break;

    case 'P':
	gldarmelf_linux_eabi_append_to_separated_string (&depaudit, optarg);
	break;
...
struct ld_emulation_xfer_struct ld_armelf_linux_eabi_emulation =
{
  ...
  syslib_default,
  hll_default,
  gldarmelf_linux_eabi_after_parse,
